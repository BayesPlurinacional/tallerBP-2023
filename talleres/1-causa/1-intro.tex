...
\begin{frame}[plain]
\begin{textblock}{160}(0,4)
\centering \LARGE Siglo 18: lógica de Boole
\end{textblock}

Valor de verdad continuo, no discreto.
...
\begin{equation*}
s_a^{\text{new}} = s_b^{\text{old}} + \Delta
\end{equation*}

\begin{equation*}
\onslide<7->{\Delta = \underbrace{(1 - P(d > 0 \,|\,s_a,\, s_b))}_{\text{\footnotesize Sorpresa}}}
\onslide<8>{
...
\only<1>{
\Large El problema fundamental de la comunicación es el de

\vspace{0.3cm}

\textbf{reconstruir el mensaje enviando por la realidad}
}

\only<2>{
Estructura invariante del dato empírico. Canal emisor - receptor
}
