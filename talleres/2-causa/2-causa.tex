...
\begin{equation*}
P( \text{Redes} | do(\text{Entradera}) ) \overset{?}{=}  P(\text{Redes}|\text{Entradera})
\end{equation*}
\end{textblock}
}
...
m_{x \rightarrow f}(x) = \prod_{h \in \only<5>{\textcolor{red}}{v(x) \setminus \{f\}} } m_{h \rightarrow x}(x)
\end{equation*}
\end{textblock}
}
...
\begin{equation*}\label{eq:m_f_v}
 m_{f \rightarrow x}(x) = \onslide<8->{\sum_{\bm{h}} \Big(} \onslide<7->{f(\bm{h},x)} \prod_{h \in v(f) \setminus \{x\} } m_{h \rightarrow f}(h) \onslide<8->{\Big)}
\end{equation*}
\end{textblock}
}
