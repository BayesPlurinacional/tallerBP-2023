...
\begin{equation*}
P(\text{dato}_1|\text{Modelo}) = \only<5->{\phantom}{\sum_{\text{hipótesis}}}  P(\text{dato}_1| \text{hipótesis}, \text{Modelo}) \only<5->{\phantom}{P(\text{hipótesis} | \text{Modelo})}
\end{equation*}
\end{textblock}
}
...
\begin{align*}
y &= w_0 + w_1 \, x_1 + w_2 \, x_2^2 + w_3 \, x_3^3 \\[1cm]
\onslide<3->{& y  = \sum_{i=0}^{M-1} w_i \phi_i(\bm{x}) = \bm{w}^T \bm{\phi}(\bm{x}) \\[0.1cm]
